...
//  breakpoint
//
//  Created by Noman Hussain on 7/7/18.
...
class DataService {
    ...
    func getAllFeedOfCurrentUser(handler : @escaping (_ messageArray : [Message])->()) {
        var messageArray = [Message]()
        REF_FEED.observeSingleEvent(of: .value) { (messageSnapshot) in
            ...
            for message in messageSnapshot{
                if (message.childSnapshot(forPath: "senderId").value as! String) == Auth.auth().currentUser?.uid{
                    let content = message.childSnapshot(forPath: "content").value as! String
                    let senderId = message.childSnapshot(forPath: "senderId").value as! String
